        ...
        <div x-show="modalTambahForm" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform translate-y-1/2" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0  transform translate-y-1/2" @click.away="modalTambahForm = !modalTambahForm" @keydown.escape="modalTambahForm = !modalTambahForm" class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl">
            ...
            <form method="POST" action="{{ route('produk.store') }}">
                ...
                <label class="block text-sm">
                    ...
                    <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" name="namaproduk" placeholder="Nama Produk" />
                </label>
                ...
                <label class="block mt-4 text-sm">
                    ...
                    <select class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray" name="varian_id" placeholder="">
                            <option value="" selected hidden>Pilih Varian</option>
                        @foreach ($varian as $item)
                            <option value="{{ $item->id }}">{{ $item->varian }}</option>
                        @endforeach
                    </select>
                </label>
                ...
                <label class="block mt-4 text-sm">
                    ...
                    <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" name="harga" placeholder="Harga" />
                </label>
